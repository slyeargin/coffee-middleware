...
module.exports = coffee.middleware = function(options){
  ...
  return function(req, res, next) {
    ...
    if (regex.handle.test(pathname)) {
      ...
      var sourceMapPath = jsPath.replace('.js', '.js.map');
      var sourceMappingURL = path.basename(sourceMapPath)
      ...
      var compile = function() {
        ...
        fs.readFile(coffeePath, 'utf8', function(err, str){
          ...
          try {
            var compiled = options.compile(str, {filename: coffeePath, sourceMap: true});
            ...
            mkdirp(path.dirname(jsPath), 0777, function(err){
              if (err) return error(err);
              fs.writeFile(jsPath, compiledJS, 'utf8', next);
              fs.writeFile(sourceMapPath, compiled.v3SourceMap, 'utf8', next);
