...
module.exports = coffee.middleware = function(options){
  ...
  var coffeeError = function(err) {
    log("COFFEE " + err.type + ' error', err.message, 'error');
    log("COFFEE File", err.filename + ' ' + err.line + ':' + err.column, 'error');
  };
  ...
  return function(req, res, next) {
    ...
    if (regex.handle.test(pathname)) {
      ...
      var coffeePath = path.join(src, (
        regex.compress.test(pathname)
        ? pathname.replace(regex.compress, '.coffee')
        : pathname.replace('.js', '.coffee'
      )));
      ...
      var compile = function() {
        ...
        fs.readFile(coffeePath, 'utf8', function(err, str){
          ...
          try {
            var compiled = options.compile(str);
